...
*/
/**************************************************************************/
Adafruit_Fingerprint::Adafruit_Fingerprint(uint32_t password, int baudrate) {
  thePassword = password;
  theAddress = 0xFFFFFFFF;

  mySerial = new BufferedSerial(D1, D0, baudrate);
}
...
void Adafruit_Fingerprint::writeStructuredPacket(
  ...
  uint8_t data = (uint8_t)(packet.start_code >> 8);
  mySerial->write(&data, 1);
  data = (uint8_t)(packet.start_code & 0xFF);
  mySerial->write(&data, 1);
  mySerial->write(&packet.address[0], 1);
  mySerial->write(&packet.address[1], 1);
  mySerial->write(&packet.address[2], 1);
  mySerial->write(&packet.address[3], 1);
  mySerial->write(&packet.type, 1);

  uint16_t wire_length = packet.length + 2;
  data = (uint8_t)(wire_length >> 8);
  mySerial->write(&data, 1);
  ...
  for (uint8_t i = 0; i < packet.length; i++) {
    mySerial->write(&packet.data[i], 1);
    sum += packet.data[i];
#ifdef FINGERPRINT_DEBUG
    Serial.print(", 0x");
